...
namespace UXAV.AVnetCore.Logging
{
    public static class Logger
    {
            ...
            ConsoleConnection connection, CommandResponseAction respond)
        {
            lock (CommandsDict)
            {
                if (args.ContainsKey("command") && CommandsDict.ContainsKey(args["command"]))
                {
                    ...
                    respond($"  {Ansi.BrightCyan}{c.CommandName}{Ansi.Reset}    {c.Description}\r\n");
                    ...
                    {
                        respond($"      -{Ansi.Cyan}{argName}{Ansi.Reset}\r\n");
                    }
                ...
                {
                    respond($"Unknown command \"{Ansi.BrightRed}{args["command"]}{Ansi.Reset}\"");
                    return;
                }
                ...
                respond("\r\n" + Ansi.Blue + header + Ansi.Reset + "\r\n");

                respond(
                    "  " + Ansi.White + "Command".PadRight(nameColLen + 2) + "  " + "Description" + Ansi.Reset +
                    "\r\n");
                var line = "  ";
                ...
                respond(Ansi.Blue + line + Ansi.Reset + "\r\n");
                foreach (var c in CommandsDict.Values.OrderBy(c => c.CommandName))
                {
                    respond(
                        $"  {Ansi.BrightCyan}{c.CommandName.PadRight(nameColLen + 2)}{Ansi.Reset}  {c.Description}\r\n");
                }
            }
        }
            ...
            ConsoleConnection connection, CommandResponseAction respond)
        {
            if (CrestronEnvironment.DevicePlatform == eDevicePlatform.Appliance)
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    table.AddRow(ipId.ToString("X2"), model, deviceType, deviceId, port, ipAddress,
                        status ? Ansi.Green + "ONLINE " + Ansi.Reset : Ansi.White + "OFFLINE" + Ansi.Reset,
                        description);
                }
            ...
            /*
            respond($"IP Table for app instance: \"{InitialParametersClass.RoomId}\":\r\n");

            JToken data = null;
            try
            {
                data = Vc4WebApi.GetIpTable();
            }
            catch (Exception e)
            {
                Error("Error trying to process IP Table from internal API: {0}", e.Message);
                return;
            }

            if (data != null)
            {
                var table = new ConsoleTable("IP ID", "Model", "Device Type", "Unique Id", "Status", "Hostname",
                    "Remote IP", "MAC Address", "Supports Association", "Description");

                foreach (var entry in data.OrderBy(e => e["ProgramIpId"]!.Value<uint>()))
                {
                    var ipId = entry["ProgramIpId"]!.Value<uint>();
                    var deviceType = entry["device_type"]!.Value<uint>();
                    var model = entry["Model"]!.Value<string>();
                    var description = entry["Description"]!.Value<string>();
                    var status = entry["Status"]!.Value<string>() == "ONLINE";
                    var uniqueId = entry["UniqueId"]!.Value<uint>();
                    var remoteIp = entry["remote_ip"]!.Value<string>();
                    var macAddress = entry["MacAddress"]!.Value<string>();
                    var hostname = entry["Hostname"]!.Value<string>();
                    var association = entry["SupportAssociation"]!.Value<bool>();
                    table.AddRow(ipId.ToString("X2"), model, deviceType, uniqueId,
                        status ? Ansi.Green + "ONLINE " + Ansi.Reset : Ansi.White + "OFFLINE" + Ansi.Reset, hostname,
                        remoteIp, macAddress, association, description);
                }
            ...
            try
            {
                data = Vc4WebApi.GetDeviceMap();
            }
            catch (Exception e)
            {
                Error("Error trying to process DeviceMap from internal API: {0}", e.Message);
                return;
            }

            if (data != null)
            {
                var table = new ConsoleTable("Device IP ID", "App IP ID", "Model", "Unique Id", "Status", "Hostname",
                    "MAC Address", "Supports Association", "Name", "Description");

                foreach (var entry in data.Values().OrderBy(e => e["DeviceId"].Value<uint>())
                    .Where(e => e["ProgramInstanceId"].Value<string>() == InitialParametersClass.RoomId ||
                                string.IsNullOrEmpty(e["ProgramInstanceId"].Value<string>())))
                {
                    var deviceId = entry["DeviceId"].Value<uint>();
                    var programIpId = entry["ProgramIpId"].Value<uint>();
                    var model = entry["Model"].Value<string>();
                    var description = entry["Description"].Value<string>();
                    var status = entry["Status"].Value<string>() == "ONLINE";
                    var uniqueId = entry["UniqueId"].Value<uint>();
                    ...
                    var association = entry["SupportAssociation"].Value<bool>();
                    table.AddRow(deviceId.ToString("X2"), programIpId.ToString("X2"), model, uniqueId,
                        status ? Ansi.Green + "ONLINE " + Ansi.Reset : Ansi.White + "OFFLINE" + Ansi.Reset, hostname,
                        macAddress, association, name, description);
                }

                respond("\r\nCurrent device map (unassociated or linked to this program instance):\r\n\r\n" +
                        table.ToString(true));
            }*/
        }
        ...
        private static void WriteLog(LoggerMessage message)
        {
            ...
            EventService.Notify(EventMessageType.LogEntry, message);
