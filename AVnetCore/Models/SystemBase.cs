...
using UXAV.AVnetCore.WebScripting.Dashboard;
...
namespace UXAV.AVnetCore.Models
{
    public abstract class SystemBase
    {
        ...
        protected SystemBase(CrestronControlSystem controlSystem)
        {
            ...
            Logger.Highlight("Loading other web app handlers");
            try
            {
                WebAppServer = new WebScriptingServer(this, "a");
                WebAppServer.AddRoute(@"/a/test", typeof(DashboardHandler));
                WebAppServer.AddRoute(@"/a/login", typeof(LoginHander));
                WebAppServer.AddRoute(@"/a/logout", typeof(LogoutHandler));
            }
            catch (Exception e)
            {
                Logger.Error("Could not load misc web scripting server, {0}", e.Message);
                UpdateBootStatus(EBootStatus.DidNotBoot, $"{GetType().Name}.CTOR FAIL, {e.Message}", 0);
                return;
            }
