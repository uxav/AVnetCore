...
{
    public static class BouncyCertificate
    {
        public static string CertificatePassword { get; set; } = "password";
        ...
        public static X509Certificate2 CreateSelfSignedCertificate(string subjectName, string[] subjectAlternativeNames,
            KeyPurposeID[] usages)
        {
            // It's self-signed, so these are the same.
            var issuerName = subjectName;
        ...
        public static void WriteCertificate(X509Certificate2 certificate, string outputDirectory, string certName)
        {
            ...
            {
                Logger.Error($"Failed to write x509 cert pfx\r\n{ex.Message}");
            }
            ...
            using (var writer = new StreamWriter($"{Path.Combine(outputDirectory, certName)}.cer", false))
            {
                ...
                {
                    Logger.Error($"Failed to write x509 cert cer\r\n{ex.Message}");
                }
            }
        }
            ...
            StoreLocation sl)
        {
            ...
            {
                Logger.Error($"AddCertToStore Failed\r\n{ex.Message}\r\n{ex.StackTrace}");
            }
