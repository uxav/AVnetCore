namespace UXAV.AVnet.Core.UI
{
    public static class UIMediaIcons
    {
        public const string Alert = "Alert";
        public const string AudioNote = "Audio Note";
        public const string Bluray = "Blu-ray";
        public const string Bolt = "Bolt";
        // ReSharper disable once InconsistentNaming
        public const string CD = "CD";
        public const string Check = "Check";
        public const string Climate = "Climate";
        public const string Delete = "Delete";
        public const string DownAlt = "Down Alt";
        public const string Down = "Down";
        public const string Eject = "Eject";
        public const string Enter = "Enter";
        public const string Film = "Film";
        public const string Fwd = "Fwd";
        public const string Home = "Home";
        public const string LeftAlt = "Left Alt";
        public const string Left = "Left";
        public const string Lights = "Lights";
        public const string Live = "Live";
        public const string Minus = "Minus";
        public const string NextPage = "Next Page";
        public const string Next = "Next";
        public const string Pause = "Pause";
        public const string Phone = "Phone";
        public const string Play = "Play";
        public const string PlayPause = "Play-Pause";
        public const string Plus = "Plus";
        public const string Power = "Power";
        public const string PrevPage = "Prev Page";
        public const string Previous = "Previous";
        public const string Rec = "Rec";
        public const string Repeat = "Repeat";
        public const string Replay = "Replay";
        public const string Req = "Rew";
        public const string RightAlt = "Right Alt";
        public const string Right = "Right";
        // ReSharper disable once InconsistentNaming
        public const string RSS = "RSS";
        public const string Shuffle = "Shuffle";
        public const string Stop = "Stop";
        public const string Theater = "Theater";
        public const string ThumbDown = "Thumb Down";
        public const string ThumbUp = "Thumb Up";
        public const string Triangle = "Triangle";
        public const string UpAlt = "Up Alt";
        public const string Up = "Up";
        public const string VideoScreen = "Video Screen";
        public const string VolumeHi = "Volume Hi";
        public const string VolumeLo = "Volume Lo";
        public const string VolumeMute = "Volume Mute";
        public const string AddressBook = "Address Book";
        public const string Alarm = "Alarm";
        public const string Calendar = "Calendar";
        public const string Clock = "Clock";
        public const string Eye = "Eye";
        public const string Game = "Game";
        public const string Gear = "Gear";
        public const string Globe = "Globe";
        public const string Help = "Help";
        public const string Image = "Image";
        public const string Info = "Info";
        public const string Keypad = "Keypad";
        public const string MagnifyingGlass = "Magnifying Glass";
        public const string Mic = "Mic";
        public const string PhoneDown = "Phone Down";
        public const string Playlist = "Playlist";
        public const string SnowFlake = "Snow Flake";
        public const string Sun = "Sun";
        public const string Users = "Users";
        public const string Door = "Door";
        public const string Drapes = "Drapes";
        public const string Fire = "Fire";
        // ReSharper disable once InconsistentNaming
        public const string IPad = "iPad";
        // ReSharper disable once InconsistentNaming
        public const string IPhoneIPodTouch = "iPhone-iPod Touch";
        // ReSharper disable once InconsistentNaming
        public const string IPod = "iPod";
        public const string MicMute = "Mic Mute";
        public const string PadlockClosed = "Padlock Closed";
        public const string PadlockOpen = "Padlock Open";
        public const string Pool = "Pool";
        public const string Settings = "Settings";
        public const string Shades = "Shades";
        public const string Share = "Share";
        public const string Shield = "Shield";
        public const string Slow = "Slow";
        public const string TV = "TV";
        public const string User = "User";
        public const string WiFi = "Wi-Fi";
        public const string RepeatItem = "Repeat Item";
        public const string RepeatOff = "Repeat Off";
        public const string ShuffleItem = "Shuffle Item";
        public const string ShuffleOff = "Shuffle Off";
        public const string SongAdd = "Song Add";
        public const string Star = "Star";
        public const string UserBookmark = "User Bookmark";
        public const string PlayAll = "Play All";
        public const string PlayAlt = "Play Alt";
        public const string PlayLibrary = "Play Library";
        public const string PlayList = "Play List";
        public const string Weather = "Weather";
        public const string Projector = "Projector";
        public const string Camera = "Camera";
        public const string DownloadCloud = "Download Cloud";
        public const string RadioSignal = "Radio Signal";
        public const string Satellite = "Satellite";
        public const string Laptop = "Laptop";
        public const string DVD = "DVD";
        public const string Pen = "Pen";
        public const string Brush = "Brush";
        public const string CheckboxChecked = "Checkbox Checked";
        public const string CheckboxOff = "Checkbox Off";
        public const string List = "List";
        public const string Android = "Android";
        public const string Apple = "Apple";
        public const string BatteryLow = "Battery Low";
        public const string BatteryCharging = "Battery Charging";
        public const string BatteryEmpty = "Battery Empty";
        public const string BatteryFull = "Battery Full";
        public const string Bluetooth = "Bluetooth";
        public const string Brightness = "Brightness";
        public const string Cart = "Cart";
        public const string ConnectorPlate = "Connector Plate";
        public const string Connector = "Connector";
        public const string Contrast = "Contrast";
        public const string Dashboard = "Dashboard";
        public const string DeleteAlt = "Delete Alt";
        public const string Download = "Download";
        public const string Garage = "Garage";
        public const string GraphAlt = "Graph Alt";
        public const string Graph = "Graph";
        public const string Grid = "Grid";
        public const string Guide = "Guide";
        public const string HD = "HD";
        public const string HotTub = "Hot Tub";
        public const string Keyboard = "Keyboard";
        public const string LightsOff = "Lights Off";
        public const string Lync = "Lync";
        public const string MediaServer = "Media Server";
        public const string Mouse = "Mouse";
        public const string Outlet = "Outlet";
        public const string System = "System";
        public const string Trashcan = "Trashcan";
        public const string Upload = "Upload";
        public const string USBAlt = "USB Alt";
        public const string USB = "USB";
        public const string VideoInput = "Video Input";
        public const string VideoOutput = "Video Output";
        public const string Windows = "Windows";
        public const string WirelessDevice = "Wireless Device";
        public const string Wrench = "Wrench";
        public const string Stopwatch = "Stopwatch";
        public const string CommentCheck = "Comment Check";
        public const string Comment = "Comment";
        public const string Crestron = "Crestron";
        public const string LastFM = "LastFM";
        public const string LocationMinus = "Location Minus";
        public const string LocationPlus = "Location Plus";
        public const string Location = "Location";
        public const string Pandora = "Pandora";
        public const string Rhapsody = "Rhapsody";
        public const string Sirius = "Sirius";
        // ReSharper disable once InconsistentNaming
        public const string SiriusXM = "SiriusXM";
        public const string Spotify = "Spotify";
        public const string UserMinus = "User Minus";
        // ReSharper disable once InconsistentNaming
        public const string XM = "XM";
        public const string UserCheck = "User Check";
        public const string Disk = "Disk";
        public const string Ban = "Ban";
        public const string Heart = "Heart";
        public const string DND = "DND";
        public const string Eraser = "Eraser";
        public const string Blank = "Blank";
        public const string MicMuted = "Mic Muted";
        public const string VolumeMuted = "Volume Muted";
        public const string OptionsOff = "Options Off";
        public const string BrightnessMedium = "Brightness Medium";
        public const string BrightnessMax = "Brightness Max";
        public const string Folder = "Folder";
        public const string DNDOn = "DND On";
        public const string OptionsOn = "Options On";
        public const string NetworkWiFiOff = "Network Wi-Fi Off";
        public const string NetworkWiFiLow = "Network Wi-Fi Low";
        public const string NetworkWiFiMed = "Network Wi-Fi Med";
        public const string NetworkWiFiMax = "Network Wi-Fi Max";
    }
}